...
class AssetsStatusViewModelTest: XCTestCase {
    ...
    func test_네트워크정상_입출금_불가() throws {
        let service = MockAssetsStatusAPIService(isSuccess: true,
                                                 assetsState: .init(data: AssetsStatusData(depositStatus: 0,
                                                                                           withdrawalStatus: 0)))
    ...
    func test_네트워크정상_입금불가_출금가능() throws {
        let service = MockAssetsStatusAPIService(isSuccess: true,
                                                 assetsState: .init(data: AssetsStatusData(depositStatus: 1,
                                                                                           withdrawalStatus: 0)))
        let viewmodel = AssetsStatusViewModel(service: service, symbol: "")
    ...
    func test_네트워크연결실패_요청실패() throws {
        let service = MockAssetsStatusAPIService(isSuccess: false,
                                                 assetsState: .init(data: AssetsStatusData(depositStatus: 1,
                                                                                           withdrawalStatus: 1)))
        let viewmodel = AssetsStatusViewModel(service: service, symbol: "")
    ...
    func test_네트워크정상_비정상데이터_AssetsStatusData() throws {
        let service = MockAssetsStatusAPIService(isSuccess: true,
                                                 assetsState: .init(data: AssetsStatusData(depositStatus: 123123,
